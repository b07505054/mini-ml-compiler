#!/usr/bin/env python3
"""Stage 16: Pi benchmark matrix for 3 new domains (cube64, altk, rect),
each at uk1/uk2/uk4. Reuses Stage 13's low-level Pi orchestration
functions (ssh_run, build_on_pi, run_harness, aggregate_measurement_groups,
classify_runtime) UNMODIFIED -- objects were already compiled through the
Stage 15 selector's manual mode (see the compiled/ directory), this script
only transfers/builds/runs them on the Pi with interleaved measurement
groups, matching the Stage 13 methodology exactly.
"""
import json
import os
import sys
...
CANDIDATES = [
    ("cube64_uk1", "64x64x64", 8, 8, 8, 1, 500, 100, 200),
    ("cube64_uk2", "64x64x64", 8, 8, 8, 2, 500, 100, 200),
    ("cube64_uk4", "64x64x64", 8, 8, 8, 4, 500, 100, 200),
    ("altk_uk1", "32x32x32", 8, 8, 4, 1, 2000, 200, 500),
    ("altk_uk2", "32x32x32", 8, 8, 4, 2, 2000, 200, 500),
    ("altk_uk4", "32x32x32", 8, 8, 4, 4, 2000, 200, 500),
    ("rect_uk1", "32x64x32", 8, 8, 8, 1, 1000, 200, 200),
    ("rect_uk2", "32x64x32", 8, 8, 8, 2, 1000, 200, 200),
    ("rect_uk4", "32x64x32", 8, 8, 8, 4, 1000, 200, 200),
]

DOMAINS = {
    "cube64": ["cube64_uk1", "cube64_uk2", "cube64_uk4"],
    "altk": ["altk_uk1", "altk_uk2", "altk_uk4"],
    "rect": ["rect_uk1", "rect_uk2", "rect_uk4"],
}


def main():
    out_dir = ART
    pi_dir = f"/tmp/stage16_multidomain_pi_{int(time.time())}"
    ...
    with open(os.path.join(out_dir, "pi_multidomain_results.json"), "w") as f:
        json.dump({"pi_environment": env, "candidates": results}, f, indent=2)

    print(f"\nWrote {os.path.join(out_dir, 'pi_multidomain_results.json')}", file=sys.stderr)
    for label, r in results.items():
        print(f"{label}: pass={r['correctness_pass']} median={r['benchmark_aggregate']['median_of_medians_ms']}", file=sys.stderr)
