    ...
    for (const auto& node : graph.nodes) {
        ...
        if (node.op == OpType::Attention) {
            ...
            if (Q.shape.size() != 2 || K.shape.size() != 2 || V.shape.size() != 2 || Out.shape.size() != 2) {
                std::cerr << "[Verifier] Attention expects 2D tensors\n";
                ok = false;
            } else if (Q.shape != K.shape || Q.shape != V.shape || Q.shape != Out.shape) {
                std::cerr << "[Verifier] Attention expects Q, K, V, Out to have same shape\n";
                ok = false;
            }
        }
        ...
        if (node.op == OpType::CausalAttention) {
            ...
            if (Q.shape.size() != 2 || K.shape.size() != 2 || V.shape.size() != 2 || Out.shape.size() != 2) {
                std::cerr << "[Verifier] CausalAttention expects 2D tensors\n";
                ok = false;
            } else if (Q.shape != K.shape || Q.shape != V.shape || Q.shape != Out.shape) {
                std::cerr << "[Verifier] CausalAttention expects Q, K, V, Out to have same shape\n";
                ok = false;
            }
        }
        if (node.op == OpType::FusedAttention) {
            ...
            if (Q.shape.size() != 2 || K.shape.size() != 2 || V.shape.size() != 2 || Out.shape.size() != 2) {
                std::cerr << "[Verifier] FusedAttention expects 2D tensors\n";
                ok = false;
            } else if (Q.shape != K.shape || Q.shape != V.shape || Q.shape != Out.shape) {
                std::cerr << "[Verifier] FusedAttention expects Q, K, V, Out to have same shape\n";
                ok = false;
            }
        }
