// RUN: mlir-opt %s --load-pass-plugin=%plugin --pass-pipeline='builtin.module(hir-canonicalize,matmul-bias-relu-fusion)' | FileCheck %s

func.func @main(
  %A: tensor<16x128xf32>,
  %B: tensor<128x64xf32>,
  %bias: tensor<16x64xf32>
) -> tensor<16x64xf32> {
  %empty = tensor.empty() : tensor<16x64xf32>
  %zero = arith.constant 0.0 : f32

  %mm = linalg.matmul
    ins(%A, %B : tensor<16x128xf32>, tensor<128x64xf32>)
    outs(%empty : tensor<16x64xf32>) -> tensor<16x64xf32>

  %add_zero = linalg.map
      ins(%mm : tensor<16x64xf32>)
      outs(%empty : tensor<16x64xf32>)
      (%x: f32) {
    %y = arith.addf %x, %zero : f32
    linalg.yield %y : f32
  }

  %add_bias = linalg.map
      ins(%add_zero, %bias : tensor<16x64xf32>, tensor<16x64xf32>)
      outs(%empty : tensor<16x64xf32>)
      (%x: f32, %b: f32) {
    %y = arith.addf %x, %b : f32
    linalg.yield %y : f32
  }

  %relu = linalg.map
      ins(%add_bias : tensor<16x64xf32>)
      outs(%empty : tensor<16x64xf32>)
      (%x: f32) {
    %y = arith.maximumf %x, %zero : f32
    linalg.yield %y : f32
  }

  return %relu : tensor<16x64xf32>
}

// CHECK: linalg.matmul
// CHECK-SAME: fusion.candidate = "matmul_bias_relu"
// CHECK-SAME: fusion.group = "matmul_bias_relu_0"
// CHECK-SAME: fusion.role = "producer"
// CHECK-NOT: %{{.*}} = linalg.map
// CHECK: linalg.map
// CHECK-SAME: fusion.group = "matmul_bias_relu_0"
// CHECK-SAME: fusion.role = "bias_add"
// CHECK: linalg.map
// CHECK-SAME: fusion.group = "matmul_bias_relu_0"
// CHECK-SAME: fusion.role = "activation"
