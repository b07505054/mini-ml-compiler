module attributes {
  ...
  func.func @attention_decode(%qinput: tensor<1x2x1x8xf32>, %history: tensor<1x2x7x8xf32>) -> tensor<1x2x1x8xf32> {
    %q = "llm.q_proj"(%qinput) : (tensor<1x2x1x8xf32>) -> tensor<1x2x1x8xf32>
    %k = "llm.k_proj"(%history) : (tensor<1x2x7x8xf32>) -> tensor<1x2x7x8xf32>
    %v = "llm.v_proj"(%history) : (tensor<1x2x7x8xf32>) -> tensor<1x2x7x8xf32>
    %ck, %cv = "llm.kv_cache_read"(%k, %v) : (tensor<1x2x7x8xf32>, tensor<1x2x7x8xf32>) -> (tensor<1x2x7x8xf32>, tensor<1x2x7x8xf32>)
    %s = "llm.attention_scores"(%q, %ck) {attention.causal = true, attention.key_transposed = true, attention.scale = 0.3535533905932738 : f64} : (tensor<1x2x1x8xf32>, tensor<1x2x7x8xf32>) -> tensor<1x2x1x7xf32>
    %p = "llm.softmax"(%s) {attention.softmax_axis = -1 : i64} : (tensor<1x2x1x7xf32>) -> tensor<1x2x1x7xf32>
    %o = "llm.attention_output"(%p, %cv) : (tensor<1x2x1x7xf32>, tensor<1x2x7x8xf32>) -> tensor<1x2x1x8xf32>
