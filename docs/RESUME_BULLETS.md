...
- Added a portable accelerator-style HIR target model with tile shape, SRAM, vector alignment, and memory-hierarchy metadata consumed by MLIR lowering artifacts.
...
- Implemented target-aware MLIR/HIR lowering where fusion is guarded by legality checks and emitted HIR carries accelerator-style tile, memory, and alignment metadata.
