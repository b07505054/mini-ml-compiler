...
- Added a SparseCore-like HIR target model with tile shape, SRAM, vector alignment, sparse-layout, and memory-hierarchy metadata consumed by MLIR lowering artifacts.
- Implemented target-aware dispatch descriptor export that selects SRAM- and vector-aligned tile shapes from M/N/K/dtype and records rejected tile candidates for fused MatMul and quantized MatMul.
- Upgraded MatMul-Bias-ReLU fusion into a legality-driven rewrite that checks one-use producer chains, ranked/static shapes, supported dtype, bias legality, and target tile multiples before emitting fused HIR.
...
- Added MLIR FileCheck coverage for positive and negative fusion cases, preventing false-positive fusion when the ReLU consumer is absent, the MatMul result has multiple users, target shapes are dynamic, or HIR target metadata is invalid.
...
- Implemented a CPU software-prefetch MatMul-Bias-ReLU backend candidate using `__builtin_prefetch`, benchmarked it against the existing tiled fused CPU kernel, and kept the non-prefetch fallback selected when measured p95 latency did not improve.
...
- Implemented target-aware MLIR/HIR lowering where fusion is guarded by legality checks and emitted HIR carries accelerator-style tile, memory, sparse-layout, and alignment metadata.
- Implemented hardware-aware tile planning that emits selected tile, SRAM usage, vector-alignment legality, and dispatch descriptor metadata into runtime execution plans.
...
- MatMul-Bias-ReLU fusion is legality-driven: the pass refuses multi-use producers, dynamic target shapes, unsupported dtype/layout cases, and invalid bias metadata before lowering.
