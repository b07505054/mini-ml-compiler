...
|---|---|---|---|
| No project-wide `ImplementationCandidate` | partially implemented | A1 adds a compiler-internal op-scoped type and shared codec for CandidateGenerationPass, ServingCostModelPass, and PlanSelectionPass. A2 migrates the P1D.1 ThreadSchedule decision into serial/parallel candidates. Triton, AWQ/vLLM, TilePlan, KernelSelection-as-candidate, deployment, and serving candidates remain outside it. | Prevents unified candidate/provider/policy architecture beyond the active internal paths. |
| Separate candidate systems outside A2 core | implemented but unintegrated | CandidateGenerationPass/ServingCostModelPass/PlanSelectionPass and P1D.1 ThreadSchedule now use the internal candidate core, but Triton, CPU descriptors, AWQ deployment, TilePlan, QuantizationDecision, and BackendDecision still differ. | Causes parallel decision systems outside the A2 scope. |
