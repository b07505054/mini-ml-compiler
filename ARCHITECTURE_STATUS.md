...
| AArch64 native codegen: LLVM machine-scheduling evidence + opt-in schedule-unroll selection slice (fused MatMul-Bias-ReLU only) | narrow real implementation, real LLVM MIR scheduling evidence + real Raspberry Pi 5 measurement across 6 independent shape/tile domains + opt-in compiler-driver candidate selection wired to real materialization | `artifacts/backend_codegen/aarch64_schedule_final/` (canonical summary, full 3-tier artifact manifest + checksums referencing all 6 stage directories); `DOC/result/AARCH64_SCHEDULE_UNROLL_FINAL_REPORT.md`; `tools/aarch64_schedule_candidate_model.py`, `tools/select_and_compile_aarch64_matmul_schedule.py`, `mlir_passes/transforms/tile_schedule_matmul_bias_relu.template.mlir` | project-owned contribution is (1) applying the stock Transform-dialect op `transform.loop.unroll` to the K-reduction loop at a chosen point in the existing pipeline, and (2) an opt-in Python compiler-driver selector (NOT a native C++/MLIR pass) that ranks a deliberately restricted candidate set (`schedule-unroll-k` in {1,2,4}, tiles in {8x8x8,8x8x4,4x8x8}) using provenance-tagged static and/or real Pi evidence; LLVM owns instruction selection, the pre-RA machine scheduler, and register allocation entirely unmodified throughout; `schedule-unroll-k=4` measured fastest in all 6 tested domains on Raspberry Pi 5/Cortex-A76/FP32 -- this is NOT claimed as universal, and calibrated/uk4 selection is opt-in only, never a compiler default (default mode is `manual`, i.e. today's unchanged `--schedule-unroll-k` behavior); static backend evidence alone correctly predicted the measured winner in only 2 of 6 domains (exactly the 2 where the winner is spill-free) -- exact-domain Raspberry Pi calibration, not static scoring, is required for reliable winner selection; no larger tile than 8x8x8, no target other than this one Raspberry Pi 5, and no dtype other than f32 have ever been measured |
